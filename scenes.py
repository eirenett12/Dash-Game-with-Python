...
from ui_sprites.intro import SpriteIntro
from ui_sprites.main_menu import ButtonSpriteContinue, SpriteLogo
from ui_sprites.gameplay import SpritePenguin, SpriteTree, TextSpriteScore
from ui_sprites.register import SpriteTickboxRegister, SpriteTickboxLogin, SpriteTextRegisterUsername, SpriteTextRegisterEmail, SpriteTextLoginUsername, ButtonSpriteTextInputRegisterUsername, ButtonSpriteTextInputRegisterEmail, ButtonSpriteTextInputLoginUsername, ButtonSpritePlay, TextSpriteRegisterUsername, TextSpriteRegisterEmail, TextSpriteLoginUsername, TextSpriteWarning
from ui_sprites.highscore import ButtonSpriteBack, SpriteHighscore, SpriteTable, TextSpritePlayer1, TextSpritePlayer2, TextSpritePlayer3, TextSpritePlayer4, TextSpritePlayer5, TextSpriteScorePlayer1, TextSpriteScorePlayer2, TextSpriteScorePlayer3, TextSpriteScorePlayer4, TextSpriteScorePlayer5
from assets import AssetsLoader
from dtos import SelectDTO, SwitchSceneDTO, WarningDTO
from services import DashService
from sprites import ButtonSubject
        ...
        if self.subject_output is not None:
            for dto in self.subject_output:
                if type(dto) is SwitchSceneDTO:
                    print(f"Returning DTO {dto.scene_name} to UI")
                    return dto
...
class SceneGameplay(Scene):
    ...
    # def do_once(self):
    #     Scene.do_once(self)
    #     self.dash_service.service_gameplay.reset()
    ...
    def update(self, _dt: float, _mouse_coords: tuple[int, int]):
        ...
        self.dash_service.service_gameplay.update()
        if self.dash_service.service_gameplay.player_is_jumping:
            self.spr_penguin.set_image(self.spr_penguin.assets[1])
        else:
            self.spr_penguin.set_image(self.spr_penguin.assets[0])
        self.spr_penguin.update_image(self.dash_service.service_gameplay.player_position_y)
...
class SceneHighScore(Scene):
    ...
    def do_once(self):
        ...
        for i, txt_player in enumerate(self.txt_players):
            txt_player.set_msg(temp_leaderboards[i].username)
        
        for j, txt_score_player in enumerate(self.txt_score_players):
            txt_score_player.set_msg(temp_leaderboards[j].score)